...
<cfif not structIsEmpty(FORM) and structKeyExists(FORM,"add")>
	<cfscript>
		APPLICATION.com.control.admin.typesAdd(FORM);
	</cfscript>
</cfif>
...
<script>
	...
	myMenu.idMenuInit = 'purchasetype';
	$(document).ready(function() {
		/*Jquery for form Validation*/
	   $("#AddPurchaseItemTypes").validate({
	   		messages: { 
					typename:"<span class='alert' style='font-size:16px;'><strong>&nbsp;!</strong></span>"
				}
	   });
	   $("#AddPurchaseItemTypes").hide();
	   
	   $("#addtypes").click(function(){
	   		$("#AddPurchaseItemTypes").toggle();
			
	  	});
   });
var chkTypenameUnique = function(typename){
    $.getJSON("/<cfoutput>#APPLICATION.Hardroot#</cfoutput>control/adminman.cfc", {
        method: 'chkPurchaseType',
        typename: typename,
        returnformat: 'json'
    }, function(isTypeUnique){
        if (isTypeUnique != true) {
            $("#theErrorDivID").html('<span class="alert">Purchase Item Type is valid</span>');
			$("#add").show();
        }
        else {
            $("#theErrorDivID").html('<span class="alert">Please select a new Purchase Item Type Name</span>');
			 $("#add").hide();
        }
    });
};
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
              ...
              <tr>
                ...
                <td align="left" valign="top">
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td width="82%" class="headline"><!-- InstanceBeginEditable name="page title" -->Purchase Item Types<!-- InstanceEndEditable --></td>
                    ...
                    <table width="750" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <td width="520" height="230" align="left" valign="top" class="background_white"><!-- InstanceBeginEditable name="content" -->
							<table>
                            	<tr>
                                	<td class="content"><a href="#" id="addtypes">Add Purchase Item Type</a></td>
                                </tr>
                                
                            </table>
                            <hr width="700" align="left" noshade>
                            
                            <form id="AddPurchaseItemTypes" name="AddPurchaseItemTypes" method="post" action="purchasetype.cfm">
                            	<table>
                                	<tr>
                                    	<td class="content">Purchase Type Name<span class="alert">*</span></td>
                                        <td class="content">
                                        	<input type="text" id="typename" name="typename" class="required limited"  maxlength="50" onchange="chkTypenameUnique(this.value);">
