...
<!-- InstanceBeginEditable name="MenuId" -->
	...
	<script src="../js/purchaseActions.js" type="text/javascript"></script>
...
<script>
	...
	myMenu.idMenuInit = 'purchaseactions';	
	var pathToUtil = '/<cfoutput>#APPLICATION.hardroot#</cfoutput>';	
  ...
  function() {
  	  ...
  	  $("#ShowPO").validate({
				messages: { 
					POSearchWord: "<span class='alert' style='font-size:16px;'><strong>&nbsp;!</strong></span>"
 				}
	   }); 
  }); 
</script>
            ...
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
              ...
              <tr>
                ...
                <td align="left" valign="top">
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td width="82%" class="headline"><!-- InstanceBeginEditable name="page title" -->Purchase Actions<!-- InstanceEndEditable --></td>
                    ...
                    <table width="750" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          <td width="520" height="230" align="left" valign="top" class="background_white"><!-- InstanceBeginEditable name="content" -->
					       	 <cfajaximport tags="cfform,cfinput-autosuggest,cfwindow,cfinput-datefield">
					       	  <cfparam name="refPurchaseID" default="" type="string" />
					       	  <cfif structKeyExists(URL,'refPurchaseID') and URL.refPurchaseID neq "">
								<cfset refPurchaseID = URL.refPurchaseID />
								<script>
									window.onload = function(){
										document.getElementById('ShowPO').submit();
									};
								</script>
							  </cfif>
							  <cfif not structIsEmpty(FORM) and structKeyExists(FORM,"POSearchWord")>
								<cfset refPurchaseID = FORM.POSearchWord />	
							  </cfif>
					       	  <cfform name="ShowPO" id="ShowPO" action="purchaseactions.cfm" method="post">
					       	  	<table>
					       	  		<tr>
								...
								<input name="po_id" type="hidden">
					       	  			<td>
					       	  				<cfinput type="text" class="required"
													name="POSearchWord" 
													autosuggest="cfc:#APPLICATION.hardRoot#control/purchasing.POAutoSuggest({cfautosuggestvalue})" 
													autoSuggestMinLength="5" 
													value="#refPurchaseID#"
													/>
					       	  			</td>
					       	  			<td>
					       	  			...
					       	  			<td>
					       	  				<input name="POSearch" type="submit" value="Show">
					       	  			</td>
					       	  		</tr>
					       	  	</table>
					       	  </cfform>
							<cfif not structIsEmpty(FORM) and structKeyExists(FORM,"POSearchWord")>
								<cfinclude template="purchActions_form.cfm" />	
							</cfif>
							<cfif not structIsEmpty(FORM) and structKeyExists(FORM,"updateinfo")>
								<cfset update = APPLICATION.com.control.purchasing.updatePurchase(FORM) />
								<table width="100%">
				       	  			<tr>
										<td class="title_paragraph">
											<cfoutput>#update.message#</cfoutput>
										</td>
									</tr>
								</table>
							</cfif>
						<cfwindow x="210" y="300" center="false" width="715" height="575" name="RegDisplay"
						       ...
						       headerStyle="background-color: ##006e51; color:##ffffff ; " 
						        ...
						        source="purchRegister.cfm?purchase_id={ShowPO:po_id}&item_id={ShowPO:po_item}" 
						        />
						
						<cfwindow x="210" y="300" center="false" width="715" height="575" name="PrintOrder"
						       refreshOnShow ="true" draggable="true" modal="true" bodyStyle="font-family: verdana; background-color: ##ffffff;" 
						       headerStyle="background-color: ##006e51; color:##ffffff ; " 
						        title="Print Order" initshow="false"
						        source="printorder.cfm?purchase_id={ShowPO:po_id}" 
						        />
